...
func TestOutputBridge_FlushesToPrometheus(t *testing.T) {
	...
	go runOutputBridge(ctx, ch, outputs, scoreEngine, nil, nil, 50*time.Millisecond, done, logger, nil, nil, "")
...
func TestFullPipeline_AdapterToPrometheus(t *testing.T) {
	...
	go runOutputBridge(ctx, norm.Output(), outputs, scoreEngine, nil, nil, 50*time.Millisecond, done, logger, nil, nil, "")
...
func TestOutputBridge_StateHolder(t *testing.T) {
	...
	go runOutputBridge(ctx, ch, nil, scoreEngine, stateHolder, nil, 50*time.Millisecond, done, logger, nil, nil, "")

	// Wait for at least one flush.
	time.Sleep(200 * time.Millisecond)

	batch, scores, _ := stateHolder.Get()
	if len(batch) == 0 {
